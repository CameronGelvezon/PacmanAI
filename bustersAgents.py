...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghost in livingGhostPositionDistributions:
            highest = 0
            place = None
            for prob_key in ghost.keys():
                if highest < ghost[prob_key]:
                    highest = ghost[prob_key]
                    place = prob_key
            ghost_places.append(place)
        closest = None
        dist_close = 10000000000000
        for p in ghost_places:
            current_dist = self.distancer.getDistance(pacmanPosition, p)
            if current_dist < dist_close:
                dist_close = current_dist
                closest = p
        best = None
        min_dist = 10000000000
        for act in legal:
            new = Actions.getSuccessor(pacmanPosition, act)
            current = self.distancer.getDistance(new, closest)
            if current < min_dist:
                min_dist = current
                best = act
        return best
