...
def sentence3() -> Expr:
    ...
    e1 = a1 % disjoin([conjoin([a0, ~k0]), conjoin([~a0, b0])])
    e2 = ~(a0 & b0)
    e3 = b0
    return conjoin([e1, e2, e3])
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([disjoin([~i[0], ~i[1]]) for i in list(itertools.combinations(literals, r = 2))])
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    possible_causes_sent = disjoin(possible_causes)
    curr = PropSymbolExpr(pacman_str, x, y, time=now)

    return curr % possible_causes_sent
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    wall_part = [PropSymbolExpr(wall_str, c[0], c[1]) >> ~PropSymbolExpr(pacman_str, c[0], c[1], time = t) for c in all_coords]
    place_part = [PropSymbolExpr(pacman_str, c[0], c[1], time = t) for c in all_coords if c in non_outer_wall_coords]
    direct_part = [PropSymbolExpr(way, time = t) for way in DIRECTIONS]
    pacphysics_sentences.append(conjoin(wall_part))
    pacphysics_sentences.append(exactlyOne(place_part))
    pacphysics_sentences.append(exactlyOne(direct_part))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    for i in range(2):
        current_model = pacphysicsAxioms(i, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords,
                               walls_grid=walls_grid, sensorModel=None, successorAxioms=allLegalSuccessorAxioms)
        KB.append(current_model)

    act_1 = PropSymbolExpr(action0, time=0)
    act_2 = PropSymbolExpr(action1, time=1)
    KB.append(act_1)
    KB.append(act_2)
   
    current_model = conjoin(KB)
    model_0 = findModel(conjoin(PropSymbolExpr(pacman_str, x1, y1, time=1), current_model))
    model_1 = findModel(conjoin(~PropSymbolExpr(pacman_str, x1, y1, time=1), current_model))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(t)
        place_list = [PropSymbolExpr(pacman_str, coord[0], coord[1], time = t) for coord in non_wall_coords]
        if t > 0:
            succ = conjoin([pacmanSuccessorAxiomSingle(coord[0], coord[1], time = t, walls_grid = walls_grid) for coord in non_wall_coords])
            KB.append(succ)
        KB.append(exactlyOne(place_list))
        action_list = [PropSymbolExpr(a, time=t) for a in actions]
        KB.append(exactlyOne(action_list))
        current_model = findModel(conjoin(conjoin(KB), PropSymbolExpr(pacman_str, xg, yg, time=t)))
        if current_model is not False:
            return extractActionSequence(current_model, actions)
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        place_list = [PropSymbolExpr(pacman_str, coord[0], coord[1], time = t) for coord in non_wall_coords]
        if t > 0:
            succ = conjoin([pacmanSuccessorAxiomSingle(coord[0], coord[1], time = t, walls_grid = walls) for coord in non_wall_coords])
            KB.append(succ)
        KB.append(exactlyOne(place_list))
        action_list = [PropSymbolExpr(a, time=t) for a in actions]
        KB.append(exactlyOne(action_list))
        current_model = findModel(conjoin(conjoin(KB), disjoin([PropSymbolExpr(food_str, c[0], c[1], time=t) for c in food])))
        if current_model is False:
            return extractActionSequence(current_model, actions)
    "*** END YOUR CODE HERE ***"
