...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for b in range(Y_RANGE):
            possible_locations.append((i,b))
    variableDomainsDict[PAC] = possible_locations
    variableDomainsDict[GHOST0] = possible_locations
    variableDomainsDict[GHOST1] = possible_locations
    distance = []
    for i in range(X_RANGE + Y_RANGE + 6):
        distance.append(i)
    variableDomainsDict[OBS0] = distance
    variableDomainsDict[OBS1] = distance
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            cpts, joinedFactor = joinFactorsByVariable(cpts, var)
            if len(joinedFactor.unconditionedVariables()) != 1:
                cpts.append(eliminate(joinedFactor, var))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        manh = manhattanDistance(pacmanPosition, ghostPosition)

        obs = busters.getObservationProbability(noisyDistance, manh)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for location in self.allPositions:
            self.beliefs[location] = self.getObservationProb(current, pac, location, jail) * self.beliefs[location]
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        better = DiscreteDistribution()
        main = {}
        for location in self.allPositions:
            newPostDict = self.getPositionDistribution(gameState, location)
            main[location] = newPostDict
        for location in self.allPositions:
            current = main[location]
            for x, y in current.keys():
                newPos = (int(x), int(y))
                better[newPos] = better[newPos] + (current[(x,y)] * self.beliefs[location])
        for key in self.beliefs.keys():
            self.beliefs[key] = better[key]
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for part in self.particles:
            if part in new.keys():
                new[part] += self.getObservationProb(observation, gameState.getPacmanPosition(), part, self.getJailPosition())
            else:
                new[part] = self.getObservationProb(observation, gameState.getPacmanPosition(), part, self.getJailPosition())
        new.normalize()
        if new.total() == 0:
